...
X = as.matrix(matrix_data[, (ncol(matrix_data) - 4):(ncol(matrix_data) - 4)])
...
iter_function = function(i,X,Y)
{
  ...
  R.forest <-multi_regression_forest(train_X,train_Y,num.trees = 40000)
  R2.forest <-multi_regression_forest(train_X,train_Y2)
  ...
  rf.soft = thresholding[which.min(error.thresh.soft[2:length(error.thresh.soft)])+1]
